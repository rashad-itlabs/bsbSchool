...
import 'package:bsbschool/features/auth/domain/usecases/forgot_password.dart';
...
/// Records what the reset asked of the backend, and can be told to reject
/// either of the two calls the flow makes.
class _RecordingRepository implements AuthRepository {
  final Failure? sendFailure;
  final Failure? resetFailure;

  final List<String> sentTo = [];
  final List<({String email, String password, String otp})> reset = [];
  ...
  Future<Either<Failure, Unit>> resetPassword({
    ...
    reset.add((email: email, password: password, otp: otp));
...
(ForgotPasswordCubit, _RecordingRepository) _cubit({
  ...
  return (
    ForgotPasswordCubit(
      sendResetCode: ForgotPassword(repository),
...
void main() {
  test('the address, then the mailed code, then the password', () async {
    ...
    cubit.continueWithCode('123456');
    expect(cubit.state.step, ForgotPasswordStep.password);
    // The digits are only carried here; nothing has gone out yet.
    ...
    // The code travels with the password — that is what the backend checks.
    expect(repository.reset.single.email, 'parent@bsb.edu.az');
    expect(repository.reset.single.password, 'yeniSifre1');
    expect(repository.reset.single.otp, '123456');
  });
  ...
  test('a rejected code is reported, and the digits can be retyped', () async {
    final (cubit, repository) = _cubit(
      resetFailure: const ValidationFailure('Kod yanlışdır və ya vaxtı bitib'),
    );
    addTearDown(cubit.close);

    await cubit.requestCode('parent@bsb.edu.az');
    cubit.continueWithCode('000000');
    await cubit.submit(password: 'yeniSifre1', passwordConfirmation: 'yeniSifre1');

    // The backend has the last word on the code, so the complaint lands here.
    expect(cubit.state.status, ForgotPasswordStatus.error);
    expect(cubit.state.message, 'Kod yanlışdır və ya vaxtı bitib');
    expect(repository.reset.single.otp, '000000');

    // And the parent is not stranded: the digits are one tap away.
    cubit.backToCode();
    expect(cubit.state.step, ForgotPasswordStep.code);
    expect(cubit.state.status, ForgotPasswordStatus.initial);
  });

  test('the password step is unreachable without six digits', () async {
    final (cubit, repository) = _cubit();
    addTearDown(cubit.close);

    await cubit.requestCode('parent@bsb.edu.az');
    // A submit asked for while the sheet is still on the digits does nothing.
    await cubit.submit(password: 'yeniSifre1', passwordConfirmation: 'yeniSifre1');
  ...
  test('a short code does not open the password step', () async {
    final (cubit, _) = _cubit();
    addTearDown(cubit.close);

    await cubit.requestCode('parent@bsb.edu.az');
    cubit.continueWithCode('123');
